...
GK <- cbind(as.data.frame(apparent_input[,1]),as.data.frame(apparent_input[,2]))
colnames(GK) <- c("genos","key")
Data <- t(as.data.frame(apparent_input[,3:ncol(apparent_input)]))
...
Out1<-Out1[which(Out1$Cross.Type != 'self'), ]
...
Out1$Parent1<-as.character(Out1$Parent1)
GK$genos<-as.character(GK$genos)

for (i in 1:nrow(GK)) {
  
  genoOut <- Out1
  Geno <- append(Geno,as.character(GK$genos[i]))
  MeanGDGeno <- append(MeanGDGeno,mean(genoOut$GD[i]))
  MinGDGeno <- append(MinGDGeno,min(genoOut$GD[i]))
  MaxGDGeno <- append(MaxGDGeno,max(genoOut$GD[i]))
  MeanSNPsGeno <- append(MeanSNPsGeno,mean(genoOut$SNPs[i]))
}
...
Out1b <- Out1a[which(Out1a$GD <= .1),]
Out1b <- Out1b[which(Out1b$SNPs > 300),]
...
if (TDtGap$p.value < .01) {
  ...
  if (nrow(L) > 0) {    
    ...
    Out2a <- Out2[which(Out2$p.value < .01),]
...
# Print the Triad analysis plots
{
  ...
  SortGD$Colour[SortGD$GD <= ThresholdT] = "red"
    SortGD$Colour[SortGD$GD > ThresholdT] = "black"
      plot(SortGD$GD,xlab="Test triads, ordered by GDij|POk",ylab="Gower Genetic Dissimilarity (GDij|k)", main="Triad analysis plot",pch=1,cex=.5,col=SortGD$Colour,xaxt="n")
...
Pa1 <- Out1[unique(c(1,3, 6))]
Pa2 <- Out1[unique(c(2,3,6))]
...
for (i in 1:ncol(GDM)) {
  ...
  MpSig <- MpVect[which(MpVect$Mpnorm < sqrt(.01)),]
  if (nrow(MpSig) < 1) {
    next
  }
...
Out3 <- Out3[which( ((Out3$DMPv < .01) & (Out3$DCumPv < .01)) | ((Out3$DRPv < .01) & (Out3$DCumPv < .01)) ),]
...
##keep lowest GD for pairs 

library(data.table)
out3<-Out2a%>%
  group_by(Offspring)%>%
  slice(which.min(GD))#export new datatable

write.csv(out3, file="finaltrios.csv")

library(ggplot2)

ggplot(data=finaltrios, aes(x=Group, y=GD, fill=Group))+geom_boxplot(
)



##adding in kinship analysis 

#load in vcf file 

vcf<-read.vcfR('./parentage_samples.vcf')

vcf.fn <- "parentage_samples.vcf"
genindK<-vcfR2genind(vcf)
gi4<-gi2gl(genindK, parallel=FALSE)
gds4<-gl2gds(libgenlight1, outfile="Spawning.gds", outpath=".", verbose=NULL)

genofile<-snpgdsOpen(filename="Spawning.gds")
ibd.robust<-snpgdsIBDKING(genofile, autosome.only = FALSE, type=c("KING-robust", "KING-homo"))
class(genofile)

ibd<-ibd.robust$kinship
names<-ibd.robust$sample.id
rownames(ibd)<-names
colnames(ibd)<-names
closefn.gds(genofile)

ibddist<-as.matrix(ibd.robust$IBS0)
xy<-t(combn(colnames(ibd), 2))
df<-data.frame(xy, dist=ibd[xy])
genDist2<-rownames_to_column(as.data.frame(genDist),"affy_id")

write.csv(df,  file="Spawning_Kinship.csv")



ggplot(Spawning_Kinship, aes(x=Kinship, fill=Spawning_Kinship$ReefRelationship))+geom_histogram()
