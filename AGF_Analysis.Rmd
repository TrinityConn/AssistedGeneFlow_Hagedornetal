...
```{r admixture plotting prep}
##load in required tools 
library(devtools)
#install_github('royfrancis/pophelper')
library(pophelper)

#load in all q file of with both CxC and CxF crosses 

afilesF<-list.files(path="/Users/trini/Documents/PSU/Lab/SNP Chip /plate 6/Analysis/CF/Q", 
                  pattern="*Q", full.names=T)

alistF<-readQ(files=afilesF, indlabfromfile = T)

#load in all .q files with just the CxF crosses

afilesC<-list.files(path="/Users/trini/Documents/PSU/Lab/SNP Chip /plate 6/Analysis/CF/Q2", 
                    pattern="*Q", full.name=T)

alistC<-readQ(files=afilesC, indlabfromfile=T)

#load in pop file 
popfileF<-popfile_AGF_filtered__CF2
popfileC<-popfile_AGF_filtered__CC2
#change to character type 
popfileF$V1<-as.character(popfileF$V1)
popfileC$V1<-as.character(popfileC$V1)
...
#plot all K in one file for CxF

plotQ(alistF, imgoutput="join", showindlab=T, grplab=popfileF, 
      showtitle=T, showsubtitle=T, titlelab="Admixture Results of Assisted Gene Flow Recruits", subtitlelab="CuracaoxCuracao and CuracaoxFlorida Recruits", height=1.6, indlabsize=2.3, indlabheight=.08, indlabspacer=-1, barbordercolour="white", outputfilename="CxFCxCplot", imgtype="png")

#plot all K in one file for CxC 

plotQ(alistC, imgoutput="join", showindlab=T, grplab=popfileC, 
      showtitle=T, showsubtitle=T, titlelab="Admixture Results of Assisted Gene Flow Recruits", subtitlelab="Only CuracaoxFlorida Recruits", height=1.4, indlabsize=2.3, indlabheight=.08, indlabspacer=-1, barbordercolour="white", outputfilename="CxFonlyplot", imgtype="png")

```
```{r embed }

#embed png images into markdown document

knitr::include_graphics('./CxFonlyplot.png')
knitr::include_graphics('./CxFCxCplot.png')


```

```{r apparent setup, load required packages }
library(devtools)
library (vcfR)
library(tidyverse)
library(outliers)
#install_github('halelab/apparent')
#install.packages('outliers')
#load apparent script 

```


####apparent setup####
```{r process data for running input through }
#load in vcf file 

vcf<-read.vcfR("Intergen.vcf")
vcf2<-
library(vcfR)

#create a tidy list 
gt<-vcfR2tidy(vcf, single_frame=TRUE)

#extract important information into important data frame 
gt2<-as.data.frame(gt$dat)

#subset just the columns we want! Individual CEL ID, SNP ID and allele call
gt3<-gt2%>%
  select(Indiv, ID, gt_GT_alleles)

#spread into long form, so that each individual is a row and each SNP call is a column 
geno<-spread(gt3, ID, gt_GT_alleles)

#change to -/-
geno2<-merge(Fam_CEL_intergen_pop, geno, by=c("Indiv"))

View(geno2)

#change -/- to . 

geno3<-as.character(geno2)

geno3<-data.frame(lapply(geno2,function(x) { gsub ("\\.", "-/-", x)}))
apparent_input_Intergen<-geno3
View(geno3)

#filtering for SNPS
gt1<-vcfR2tidy(vcf, info_only = FALSE, single_frame=TRUE, toss_INFO_column = TRUE, alleles=FALSE)
gt1<-as.data.frame(gt1$dat)
gt4<-gt1%>%
  select(Indiv, ID, gt_GT)

geno2<-spread(gt4, ID, gt_GT)
geno2[geno2=='0/0']<-0
geno2[geno2=="0/1"]<-1
geno2[geno2=='1/1']<-2
...
geno4<-select(geno4, -c(Indiv))

remove_column<-sapply(geno4, mean)
remove_column<-names(remove_column[remove_column==2])
t2
geno6<-geno4[, setdiff(names(geno4), remove_column)]
geno5<-data.matrix(geno4, rownames.force = NA)

geno6=geno4[,!sapply(geno4, function(x) mean(x))==2]

geno5<-as.data.frame(geno5)

#merge with popfile for additional parentage info needed for apparent 
geno_file<-merge(apparent_popfile, geno, by=c("Indiv"))
 
#write tab delimited file 

write.table(geno_file, file="apparent_input.txt", sep="\t", row.names = FALSE)
app_in_3<-apparent_input[, -grep(". ", colnames(apparent_input))]

cond<-sapply(apparent_input, function(col) )

```

####apparent with just interregional individuals####
```{r with just fam and all}
...
#run apparent
  ...
  GK <- cbind(as.data.frame(fam_apparent_input_2[,1]),as.data.frame(fam_apparent_input_2[,2]))
  colnames(GK) <- c("genos","key")
  Data <- t(as.data.frame(fam_apparent_input_2[,3:ncol(fam_apparent_input_2)]))
  ...
  # Print the Triad analysis plots
  {
    ...
    plot (SortGD$GD,xlab="Test triads, ordered by GDij|POk",ylab="Gower Genetic Dissimilarity (GDij|k)",
          main="Triad analysis plot excluding CURxCUR Offspring",pch=1,cex=.5,col=SortGD$Colour,xaxt="n")
